...
then
    ...
    local projectVersion=$(mvn help:evaluate -Dexpression=project.version | grep -e '^[^\[]')
    git merge -X theirs --allow-unrelated-histories -m "New application changes from Stratio version ${projectVersion}" master
    local newProjectVersion=${projectVersion}-SNAPSHOT
    mvn versions:set -DgenerateBackupPoms=false -DnewVersion=${newProjectVersion}
    git add .
    git commit -m "Set app version to ${newProjectVersion}"
    git push stratio $REPOS_INTEGRATION_BRANCH
    cd .. && rm -rf "${WORKDIR}"
